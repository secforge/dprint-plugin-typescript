...
fn should_skip_paren_expr<'a>(node: &'a ParenExpr<'a>, context: &Context<'a>) -> bool {
  ...
  if is_type_assertion(node.expr.kind()) {
    // Keep parens when followed by member/optional access/call: (expr as Type).member or (expr as Type)?.optional or (expr as Type)()
    if matches!(
      parent.kind(),
  ...
  if matches!(node.expr, Expr::Assign(_))
    && matches!(
      parent.kind(),
      NodeKind::MemberExpr | NodeKind::OptChainExpr | NodeKind::CallExpr | NodeKind::OptCall | NodeKind::NewExpr
    )
  {
    return false;
  }
