...
/// Check if a node kind is a type assertion (as, satisfies, const assertion, type assertion)
/// Note: TsNonNullExpr (!) is NOT included; it's handled separately where needed
fn is_type_assertion(kind: NodeKind) -> bool {
  matches!(
    kind,
    NodeKind::TsAsExpr | NodeKind::TsSatisfiesExpr | NodeKind::TsConstAssertion | NodeKind::TsTypeAssertion
  )
}
...
fn gen_new_expr<'a>(node: &NewExpr<'a>, context: &mut Context<'a>) -> PrintItems {
  ...
  // Generate parentheses based on useParentheses mode:
  // - Disambiguation: always add parens (even if callee is parenthesized)
  // - Maintain: keep as-is (only add if args are present in the AST)
  // - PreferNone: only add if needed for member/optional access
  let should_generate_parens = match context.config.use_parentheses {
    UseParentheses::Disambiguation => true,
    UseParentheses::Maintain => node.args.is_some(),
    UseParentheses::PreferNone => is_node_accessed_by_member_or_chain(node.parent()),
  };

  // Generate arguments if present or if should_generate_parens is true
  let nodes: Vec<_> = node.args.as_ref()
    .map(|args_ref| args_ref.iter().map(|&n| n.into()).collect())
    .unwrap_or_default();

  if !nodes.is_empty() || should_generate_parens {
    items.extend(gen_parameters_or_arguments(
      GenParametersOrArgumentsOptions {
        node: node.into(),
...
/// Walks up the tree from the given node to find the first ClassDecl or ClassExpr ancestor.
fn find_ancestor_class<'a>(node: Node<'a>) -> Option<&'a Class<'a>> {
  for ancestor in node.ancestors() {
    match ancestor {
      Node::ClassDecl(class_decl) => return Some(class_decl.class),
      Node::ClassExpr(class_expr) => return Some(class_expr.class),
      _ => {}
    }
  }
  None
...
/// Left-associative operators requiring right-side parens when nested.
fn is_non_commutative_or_comparison(op: &BinaryOp) -> bool {
  matches!(
    op,
    BinaryOp::Add // Not associative due to type coercion: "x" + (4 + 2) ≠ "x" + 4 + 2
      | BinaryOp::Mul // Not associative due to floating-point precision
      | BinaryOp::Div
      | BinaryOp::Mod
      | BinaryOp::Sub
      | BinaryOp::LShift
...
/// Checks if a node (or any ParenExpr wrapping it) is accessed via member/optional chaining.
fn is_node_accessed_by_member_or_chain(mut node: Node) -> bool {
  loop {
    match node.kind() {
      NodeKind::MemberExpr | NodeKind::OptChainExpr => return true,
      NodeKind::ParenExpr => match node.parent() {
...
fn should_skip_paren_expr<'a>(node: &'a ParenExpr<'a>, context: &Context<'a>) -> bool {
  ...
  // Keep parens when parent is unary and child has lower precedence than unary operators
  // Examples: !(a ? b : c), !(a, b), !(x => x), !(a = b), !(yield a)
  if parent.kind() == NodeKind::UnaryExpr
    && matches!(
      node.expr,
      Expr::Bin(_) | Expr::Assign(_) | Expr::Cond(_) | Expr::Seq(_) | Expr::Arrow(_) | Expr::Yield(_)
    )
  {
    return false;
  }
  ...
  // Keep parens in spread with conditional expression
  // ...( cond ? a : b ) is valid, but ...cond ? a : b is parsed as (...cond) ? a : b
  if parent.kind() == NodeKind::SpreadElement && matches!(node.expr, Expr::Cond(_)) {
    return false;
  }
  ...
  // Keep parens for unary and update expressions in specific contexts
  if matches!(node.expr, Expr::Unary(_) | Expr::Update(_)) {
    // Keep parens when member accessed: (typeof x).toUpperCase(), (x++).toString()
  ...
  // Keep parens for await/yield in binary/member/call/new contexts: (await x) + 1, new (await x)
  if matches!(node.expr, Expr::Await(_) | Expr::Yield(_))
    && matches!(
      parent.kind(),
      NodeKind::BinExpr | NodeKind::MemberExpr | NodeKind::OptChainExpr | NodeKind::CallExpr | NodeKind::OptCall | NodeKind::NewExpr
    )
  {
    return false;
  }
  ...
  if let Node::BinExpr(parent_bin) = parent {
    ...
    // Keep parens for arrow functions in binary/logical expressions: a || (() => b)
    if matches!(node.expr, Expr::Arrow(_)) {
      return false;
    }

    // Keep parens when expr ends with ! and is followed by =, ==, or ===
    // to avoid ambiguity with !=, !==: (a + b!) === x should not become a + b! === x
    if parent_bin.left.range().contains(&node.range())
      && matches!(parent_bin.op(), BinaryOp::EqEq | BinaryOp::EqEqEq)
      && expr_ends_with_non_null_assertion(&node.expr)
    {
      return false;
    }

    if let Expr::Bin(inner_bin) = node.expr {
      // Keep parens when mixing logical operators (&&, ||) with nullish coalescing (??)
      // JavaScript requires parens: (a && b) ?? c or a && (b ?? c), but not a && b ?? c
      if (matches!(parent_bin.op(), BinaryOp::NullishCoalescing) && matches!(inner_bin.op(), BinaryOp::LogicalAnd | BinaryOp::LogicalOr))
        || (matches!(parent_bin.op(), BinaryOp::LogicalAnd | BinaryOp::LogicalOr) && matches!(inner_bin.op(), BinaryOp::NullishCoalescing))
      {
        return false;
      }
  ...
  // Keep parens for type assertions in various contexts
  if is_type_assertion(node.expr.kind()) {
    // Keep parens when followed by member/optional access/call/non-null: (expr as Type).member or (expr as Type)?.optional or (expr as Type)() or (expr as Type)!
    if matches!(
      parent.kind(),
      NodeKind::MemberExpr | NodeKind::OptChainExpr | NodeKind::CallExpr | NodeKind::OptCall | NodeKind::TsNonNullExpr
    ) {
      return false;
    }

    // Keep parens in extends clause: class X extends (Y as typeof Z) {}
    if let Some(class) = find_ancestor_class(node.into()) {
      if let Some(super_class) = class.super_class {
        if super_class.range().contains(&node.range()) {
          return false;
        }
      }
    }

    // Keep parens in new expression constructor: new (X as any)()
    if parent.kind() == NodeKind::NewExpr {
      if let Node::NewExpr(new_expr) = parent {
        if new_expr.callee.range().contains(&node.range()) {
          return false;
        }
      }
    }
  ...
  // Keep parens for binary expressions in various contexts
  if matches!(node.expr, Expr::Bin(_)) {
    // Member access and call have higher precedence than binary operators; (a / 1000).toString() or (a || b)(c)
    if matches!(
      parent.kind(),
      NodeKind::MemberExpr | NodeKind::OptChainExpr | NodeKind::CallExpr | NodeKind::OptCall
    ) {
      return false;
    }

    // Keep parens in new expression constructor: new (A || B)()
    if let Node::NewExpr(new_expr) = parent {
      if new_expr.callee.range().contains(&node.range()) {
        return false;
      }
  ...
  // Member access, call, non-null, and new have higher precedence than assignment, so parens are needed
  if matches!(node.expr, Expr::Assign(_))
    && matches!(
      parent.kind(),
      NodeKind::MemberExpr | NodeKind::OptChainExpr | NodeKind::CallExpr | NodeKind::OptCall | NodeKind::NewExpr | NodeKind::TsNonNullExpr
    )
  {
    return false;
  }
...
/// Check if an expression ends with a non-null assertion (!)
/// This recursively checks the rightmost part of the expression
fn expr_ends_with_non_null_assertion(expr: &Expr) -> bool {
  match expr {
    Expr::TsNonNull(_) => true,
    Expr::Paren(paren) => expr_ends_with_non_null_assertion(&paren.expr),
    Expr::Bin(bin) => expr_ends_with_non_null_assertion(&bin.right),
    Expr::Cond(cond) => expr_ends_with_non_null_assertion(&cond.alt),
    Expr::Assign(assign) => expr_ends_with_non_null_assertion(&assign.right),
    Expr::Seq(seq) => {
      if let Some(last) = seq.exprs.last() {
        expr_ends_with_non_null_assertion(last)
      } else {
        false
      }
    }
