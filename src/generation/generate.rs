...
fn contains_dangling_if(stmt: Stmt) -> bool {
  match stmt {
    ...
    Stmt::Block(block_stmt) => {
      let non_empty_stmts: Vec<_> = block_stmt.stmts.iter().filter(|s| !matches!(s, Stmt::Empty(_))).collect();
      non_empty_stmts.len() == 1 && contains_dangling_if(*non_empty_stmts[0])
    }
...
fn use_braces_for_then(then_node: Stmt, else_node: Option<Stmt>) -> bool {
  match then_node {
    Stmt::Block(block_stmt) => {
      let non_empty_stmts: Vec<_> = block_stmt.stmts.iter().filter(|s| !matches!(s, Stmt::Empty(_))).collect();
      non_empty_stmts.len() != 1 || use_braces_for_then(*non_empty_stmts[0], else_node)
    }
    Stmt::Decl(decl) => !matches!(decl, Decl::Fn(_)),
...
fn use_braces_for_else(else_node: Stmt) -> bool {
  match else_node {
    Stmt::Block(block_stmt) => {
      let non_empty_stmts: Vec<_> = block_stmt.stmts.iter().filter(|s| !matches!(s, Stmt::Empty(_))).collect();
      non_empty_stmts.len() != 1 || use_braces_for_else(*non_empty_stmts[0])
    }
    Stmt::Decl(_) => true, // All declarations need braces
    _ => false,
  }
}
...
fn gen_conditional_brace_body<'a>(opts: GenConditionalBraceBodyOptions<'a>, context: &mut Context<'a>) -> GenConditionalBraceBodyResult {
  ...
  fn get_force_braces(body_node: Node) -> bool {
    ...
    non_empty_stmts.len() != 1 || matches!(non_empty_stmts[0], Stmt::Decl(_))
  }
